...
public class QuizList extends javax.swing.JInternalFrame {
    ...
    private void tbDSCHoiMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tbDSCHoiMouseClicked
        try {
            ...
            Time examTime = (Time) model.getValueAt(row, 2);
    ...
    public void refresh() {
        ...
        jpsThoiGianLam.setValue("");
        jpsSoCauDe.setValue("");
        jpsSoCauTB.setValue("");
        jpsSoCauKho.setValue("");
    }
    ...
    public void loadQuiz() {
        ...
        try {
            while (rs.next()) {
                model.addRow(new Object[]{rs.getInt("madethi"), rs.getString("mamonhoc"), rs.getTime("time"), rs.getInt("socaudokho1"), rs.getInt("socaudokho2"), rs.getInt("socaudokho3")});
            }
            tbDSCHoi.setModel(model);
            rs.close();
        } catch (SQLException ex) {
